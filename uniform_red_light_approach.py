...
def set_red_light_random():
    red_duration = uniform(0, 10) + 20 # random value plus a constant 
...
set_left_lane('vehicle_1')
step = 0
print(f'traci simulation step size: {traci.simulation.getDeltaT()}')
begin_approach = False
end_approach = False
approach_timestep = 0
sub_results = True # This gets overwritten with a dict, its empty when vehicle leaves sim
while sub_results: # while vehicle is in simulation
